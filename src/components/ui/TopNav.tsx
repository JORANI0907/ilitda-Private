...
import {
  ...
  BarChart3, Users, Briefcase, Store, FileText,
} from 'lucide-react'
...
const BUSINESS_TABS: NavItem[] = [
  ...
  { href: '/business/form-preview', label: '신청서', icon: <FileText size={18} /> },
]
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-40 bg-surface border-b border-border-subtle h-16 hidden md:flex items-center">
      <div className="max-w-5xl mx-auto w-full px-6 flex items-center gap-8">
        {/* 로고 */}
        <Link href={`/${role}/home`} className="flex-shrink-0 flex items-center">
          <div className="h-9 w-28 flex-shrink-0">
            <Image
          ...
          {appDisplayName !== '일잇다' && (
            <span className="ml-2 text-lg font-bold text-brand-600 tracking-tight">{appDisplayName}</span>
          )}
        </Link>

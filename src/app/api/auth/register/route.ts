...
export async function POST(request: NextRequest) {
  ...
  const { email, password, name, phone, otp, role, business_name, address, business_number } = body

  if (!email?.trim() || !password || !name?.trim() || !phone?.trim() || !otp?.trim()) {
  ...
  if (role !== 'business' && role !== 'worker') {
    return NextResponse.json({ success: false, error: '역할을 선택해주세요.' }, { status: 400 })
  }
  ...
  const is_business = role === 'business'
  const is_worker = role === 'worker'
  ...
  const successResponse = NextResponse.json({ success: true, data: { role } }, { status: 201 })
